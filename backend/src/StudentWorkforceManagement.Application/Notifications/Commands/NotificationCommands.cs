...
namespace StudentWorkforceManagement.Application.Notifications.Commands;

public sealed record MarkNotificationReadCommand(Guid NotificationId) : IRequest<NotificationDto>, IAuthorizableRequest
{
    public IReadOnlyCollection<UserRole> RequiredRoles => Authorize.AnyRole;
}
public sealed record MarkAllNotificationsReadCommand : IRequest<int>, IAuthorizableRequest
{
    public IReadOnlyCollection<UserRole> RequiredRoles => Authorize.AnyRole;
}
public sealed record UpdateNotificationPreferenceCommand(NotificationPreferenceType PreferenceType, NotificationChannel Channel, bool IsEnabled) : IRequest<NotificationPreferenceDto>, IAuthorizableRequest
{
    public IReadOnlyCollection<UserRole> RequiredRoles => Authorize.AnyRole;
}

public sealed class NotificationCommandHandler(IApplicationDbContext dbContext, ICurrentUserService currentUser, IUtcClock clock)
...
{
    public async System.Threading.Tasks.Task<NotificationDto> Handle(MarkNotificationReadCommand request, CancellationToken cancellationToken)
    {
        var userId = currentUser.RequireUserId();
        var notification = await dbContext.Notifications.SingleOrDefaultAsync(entity => entity.Id == request.NotificationId && entity.UserId == userId, cancellationToken)
    ...
    public async System.Threading.Tasks.Task<int> Handle(MarkAllNotificationsReadCommand request, CancellationToken cancellationToken)
    {
        var userId = currentUser.RequireUserId();
        var notifications = await dbContext.Notifications.Where(entity => entity.UserId == userId && !entity.IsRead).ToListAsync(cancellationToken);
    ...
    public async System.Threading.Tasks.Task<NotificationPreferenceDto> Handle(UpdateNotificationPreferenceCommand request, CancellationToken cancellationToken)
    {
        var userId = currentUser.RequireUserId();
        var preference = await dbContext.NotificationPreferences.SingleOrDefaultAsync(entity => entity.UserId == userId && entity.PreferenceType == request.PreferenceType && entity.Channel == request.Channel, cancellationToken);
        if (preference is null)
        {
            preference = new NotificationPreference { Id = Guid.NewGuid(), UserId = userId, PreferenceType = request.PreferenceType, Channel = request.Channel };
            dbContext.NotificationPreferences.Add(preference);
        }
        preference.IsEnabled = request.IsEnabled;
        await dbContext.SaveChangesAsync(cancellationToken);
        return new NotificationPreferenceDto(preference.Id, preference.UserId, preference.PreferenceType, preference.Channel, preference.IsEnabled);
    }

    private static NotificationDto ToDto(Notification notification) => new(notification.Id, notification.UserId, notification.Type, notification.Title, notification.Message, notification.RelatedEntityType, notification.RelatedEntityId, notification.IsRead, notification.CreatedAt, notification.ReadAt);
}
