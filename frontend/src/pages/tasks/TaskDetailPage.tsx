...
import { assignmentModeLabels, assignmentStatusLabels, fileStatusLabels, fileStatusVariant, formatDuration, submissionStatusLabels, submissionStatusVariant } from '../../features/tasks/taskPresentation'
...
import type { FileStatus, Submission, Task, TaskCommentVisibility } from '../../features/tasks/types'
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_22rem]">
          ...
          <Tabs defaultValue="overview">
            ...
            <TabsContent value="activity" className="space-y-5">
              <Panel title="Assignment history" error={collections.history.isError} onRetry={() => void collections.history.refetch()}>
                ...
                <div className="space-y-2">{collections.history.data?.map((item) => <div key={item.id} className="rounded-md border border-border px-3 py-2 text-sm"><p className="font-medium">{assignmentModeLabels[item.mode]} · {assignmentStatusLabels[item.status]}</p><p className="text-text-secondary">{formatIstanbulDateTime(item.assignedAt)} · {item.studentName || 'Student'}{item.plannedEffortMinutes ? ` · planned ${formatDuration(item.plannedEffortMinutes)}` : ''}</p>{item.reason ? <p className="mt-1 text-text-secondary">{item.reason}</p> : null}</div>)}</div>
              </Panel>
            </TabsContent>
        ...
        <aside className="space-y-5">
          <Panel title="Assignment">
            <dl className="space-y-2 text-sm"><Row label="Assigned student" value={taskData.assignedStudentName || (taskData.assignedStudentId ? 'Assigned student' : 'Unassigned')} /><Row label="Estimate" value={formatDuration(taskData.estimatedDurationMinutes)} /><Row label="Category" value={taskData.categoryName || 'Not configured'} /></dl>
          ...
          <Panel title="Required skills" error={collections.skills.isError} onRetry={() => void collections.skills.refetch()}>
            ...
            <div className="flex flex-wrap gap-2">{collections.skills.data?.map((skill) => <Badge key={skill.id} variant="neutral">{skill.skillName ?? 'Unknown skill'} · {skill.minimumLevel}</Badge>)}</div>
          </Panel>
...
function SubmissionsPanel({ task, submissions, versions, selectedSubmission, reviewer, student, uploadError, uploadProgress, onFile, onCancelUpload, onView, onDownload, onRevision, pending }: { task: Task; submissions?: Submission[]; versions?: { id: string; versionNumber: number; fileName: string; fileSize: number; fileStatus: FileStatus; fileExtension: string; uploadedAt: string }[]; selectedSubmission?: Submission; reviewer: boolean; student: boolean; uploadError: string | null; uploadProgress: number | null; onFile: (file: File | undefined) => Promise<void>; onCancelUpload: () => void; onView: (submissionId: string, versionId: string) => void; onDownload: (submissionId: string, versionId: string) => void; onRevision: (submissionId: string, comment: string) => Promise<unknown>; pending: boolean }) {
  ...
  return (
    <Panel title="Submissions">
      ...
      <div className="space-y-3">{submissions?.map((submission, index) => <div key={submission.id} className="rounded-md border border-border px-3 py-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-medium">Submission #{index + 1}</p><Badge variant={submissionStatusVariant(submission.status)}>{submissionStatusLabels[submission.status]}</Badge></div><p className="mt-1 text-text-secondary">{submission.submittedAt ? formatIstanbulDateTime(submission.submittedAt) : 'Not submitted yet'}</p>{submission.status === 'REVISION_REQUESTED' && submission.latestReviewerComment ? <p className="mt-2 rounded-md border border-warning/30 bg-warning/10 px-3 py-2 text-text-primary">Requested changes: {submission.latestReviewerComment}</p> : null}</div>)}</div>
      {selectedSubmission ? <div className="mt-4 space-y-2"><h3 className="text-sm font-semibold">Versions</h3>{versions?.map((version) => <div key={version.id} className="rounded-md border border-border px-3 py-2 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-medium">{version.fileName}</p><p className="text-text-secondary">{formatFileKind(version.fileExtension)} · {formatFileSize(version.fileSize)} · v{version.versionNumber} · {formatIstanbulDateTime(version.uploadedAt)}</p></div>{version.fileStatus === 'CONFIRMED' ? <div className="flex gap-2"><Button type="button" size="sm" variant="outline" iconBefore={<Eye className="h-4 w-4" />} onClick={() => onView(selectedSubmission.id, version.id)}>View</Button><Button type="button" size="sm" variant="outline" iconBefore={<Download className="h-4 w-4" />} onClick={() => onDownload(selectedSubmission.id, version.id)}>Download</Button></div> : <Badge variant={fileStatusVariant(version.fileStatus)}>{fileStatusLabels[version.fileStatus]}</Badge>}</div></div>)}</div> : null}
