...
namespace StudentWorkforceManagement.Application.Tasks.Commands.AddTaskDependency;

public sealed record AddTaskDependencyCommand(Guid TaskId, Guid DependsOnTaskId) : IRequest<TaskDependencyDto>, IAuthorizableRequest
{
...
public sealed class AddTaskDependencyCommandValidator : AbstractValidator<AddTaskDependencyCommand>
{
    public AddTaskDependencyCommandValidator()
    {
        ...
        RuleFor(command => command.DependsOnTaskId).NotEmpty();
        RuleFor(command => command).Must(command => command.TaskId != command.DependsOnTaskId).WithMessage("A task cannot depend on itself.");
    }
}

public sealed class AddTaskDependencyCommandHandler(IApplicationDbContext dbContext, ITaskDependencyService dependencyService) : IRequestHandler<AddTaskDependencyCommand, TaskDependencyDto>
{
    ...
    {
        if (await dependencyService.WouldCreateCycleAsync(request.TaskId, request.DependsOnTaskId, cancellationToken))
        {
            throw new ConflictException("Task dependency would create a circular dependency.");
        }

        if (await dbContext.TaskDependencies.AnyAsync(entity => entity.TaskId == request.TaskId && entity.DependsOnTaskId == request.DependsOnTaskId, cancellationToken))
        {
            throw new ConflictException("Task dependency already exists.");
        }

        var dependency = new TaskDependency { Id = Guid.NewGuid(), TaskId = request.TaskId, DependsOnTaskId = request.DependsOnTaskId };
        dbContext.TaskDependencies.Add(dependency);
        await dbContext.SaveChangesAsync(cancellationToken);
        return new TaskDependencyDto(dependency.Id, dependency.TaskId, dependency.DependsOnTaskId);
    }
}
