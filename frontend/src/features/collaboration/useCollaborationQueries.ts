...
export function useCollaborationMutations() {
  ...
  return {
    ...
    respondMeeting: useMutation({ mutationFn: ({ id, campusPresence, availableRangesJson, note }: { id: string; campusPresence: string; availableRangesJson: string; note?: string }) => respondToMeeting(id, { campusPresence, availableRangesJson, note }), onSuccess: async (meeting) => { appToast.success('Availability submitted.'); await invalidateMeetings(meeting.id) } }),
    ...
    updateNotes: useMutation({ mutationFn: ({ id, agenda, notes }: { id: string; agenda?: string; notes?: string }) => updateMeetingNotes(id, { agenda, notes }), onSuccess: async (meeting) => invalidateMeetings(meeting.id) }),
