      ...
      IRequestHandler<GetTaskChecklistQuery, IReadOnlyCollection<TaskChecklistItemDto>>
{
    public async System.Threading.Tasks.Task<IReadOnlyCollection<TaskAssignmentHistoryDto>> Handle(GetTaskAssignmentHistoryQuery request, CancellationToken cancellationToken)
    {
        return await dbContext.TaskAssignmentHistory.AsNoTracking()
            ...
            .Select(history => new TaskAssignmentHistoryDto(history.Id, history.TaskId, history.StudentId, history.AssignedByUserId, history.AssignedAt, history.UnassignedAt, history.Status, history.Mode, history.IsActive, history.PlannedEffortMinutes, history.Reason, history.Student == null ? null : (history.Student.FirstName + " " + history.Student.LastName).Trim(), history.AssignedByUser == null ? null : history.AssignedByUser.DisplayName))
