...
describe('CourseWeeklyTimetable', () => {
  ...
  it('renders availability blocks on the correct day with human status, reason, and overlap-safe course content', () => {
    ...
    const availableBlock = within(tuesday).getByText('Available').closest('article')
    const preferredBlock = within(tuesday).getByText('Preferred').closest('article')
    const unavailableBlock = within(saturday).getByText('Unavailable').closest('article')

    expect(availableBlock).toHaveClass('border-green-200', 'bg-green-50')
    expect(preferredBlock).toHaveClass('border-blue-200', 'bg-blue-50')
    expect(unavailableBlock).toHaveClass('border-red-200', 'bg-red-50')
    expect(availableBlock?.className).not.toEqual(preferredBlock?.className)
    expect(preferredBlock?.className).not.toEqual(unavailableBlock?.className)
    expect(unavailableBlock?.className).not.toEqual(availableBlock?.className)
    expect(availableBlock).not.toHaveClass('bg-surface', 'bg-surface-secondary', 'bg-white')
    expect(preferredBlock).not.toHaveClass('bg-surface', 'bg-surface-secondary', 'bg-white')
    expect(unavailableBlock).not.toHaveClass('bg-surface', 'bg-surface-secondary', 'bg-white')
